      ...
      {isOpen && (
        <div className="absolute top-20 left-0 w-full bg-background/95 backdrop-blur-2xl border-b border-border p-6 md:hidden animate-fade-in shadow-xl">
          <div className="flex flex-col gap-6">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} className="text-lg font-medium text-foreground" onClick={() => setIsOpen(false)}>
                {item.label}
              </Link>
            ))}
            <Link href="/contact" className="text-lg font-medium text-primary" onClick={() => setIsOpen(false)}>
              Contact Us
            </Link>
          </div>
        </div>
